...
public class ValidatorRegistrationBuilder
    ...
    if (!lhs.equals(rhs)) {
        throw new IllegalArgumentException("Invalid BLS public key: does not satisfy curve equation");
    }
    ...
    // byte[] dummySignature = new byte[publicKeyBytes.length]; // Firma ficticia para validar la clave

    // // Realizar la verificación
    // int result = BLS.core_verify(dummySignature, dummyMessage, publicKeyBytes);

    // if (result != BLS.BLS_OK) {
    //     throw new IllegalArgumentException("Invalid BLS public key according to core_verify");
    // }
